...
public class MatchMapperTest extends GameServiceApplicationTests {
	...
	public void testMonthMatchesAndEvents() throws UrlException {
		 ...
		 * season.getTeams().forEach((team) -> { teamMapper.save(team);
			...
			match.getQuarters().forEach(quarter -> {
				...
				quarter.getMatchEvents().forEach(event -> {
					try {
						...
						// AKO SE DESI DA JE IGRAC PRESAO U TIM U TOKU SEZONE, PA GA NEMA U TIMOVIMA ALI
						// GA IMA
						// U ROSTERU
	...
	public void testMatchWithQuarters() {
		...
		Match m = Match.builder().matchId("SAC21015154").awayTeam(t2).homeTeam(t1).seasonYear(1910)
				.finalScore("119-120").dateTime(LocalDateTime.now().minusDays(30)).awayTeamPoints(119).build();
		matchMapper.insert(m);
		Quarter quarter1 = Quarter.builder().match(m).name("1st Q").pointsAwayTeam(119).pointsHomeTeam(110).build(),
				quarter2 = Quarter.builder().match(m).name("2nd Q").pointsAwayTeam(151).pointsHomeTeam(111).build();
	...
	public void testCrud() {
		...
		log.info(match.getMatchId() + match.getAwayTeam() + match.getFinalScore() + match.getHomeTeam());
	...
	@Test
	public void testFetchingSeasonMatches() {
		List<Match> seasonMatches = matchMapper.getSeasonMatchesWithEvents(2019);
		assertEquals(seasonMatches.size(), 1312);
		assertEquals(seasonMatches.get(3).getQuarters().size(), 4);
		assertEquals(seasonMatches.get(3).getQuarters().get(1).getMatchEvents().size(), 119);
	}
}
