	...
	private void insertEvent(MatchEvent event) {
		try {
			...
			if (event.getFirstPlayer() != null
				...
				event.getFirstPlayer().setTeam(teamMapper.getByTeamId(event.getFirstPlayer().getTeam().getTeamId()));
				...
				event.getSecondPlayer().setTeam(teamMapper.getByTeamId(event.getSecondPlayer().getTeam().getTeamId()));
