...
public class PlayerParser {
  public Player returnPlayer(Element row, Team team) {
    ...
    Player player = Player.builder().team(team).playerId(id).number(header.text())
        .name(cols.get(0).text()).position(cols.get(1).text()).height(cols.get(2).text())
        .weight(cols.get(3).text()).birthDate(date).nationality(cols.get(5).text())
        .experience(expirience).college(cols.get(7).text()).build();
    return player;
  }
