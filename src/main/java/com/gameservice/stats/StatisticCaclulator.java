...
public class StatisticCaclulator {
	...
	/*
	 * /////////////// ****************** /////////////// private static void
	 * minutesPlayed(List<ActionType[]> minutesPlayed, String quarter) {
	 * minutesPlayed.forEach(actions -> { EntersOrLeft enters = (EntersOrLeft)
	 * actions[0], left = (EntersOrLeft) actions[1]; PlayerStats stats =
	 * (PlayerStats) playersStats.get(enters.getPlayerId());
	 * stats.setTimeEntered(enters.getTime()); PlayerStats stats2 = (PlayerStats)
	 * playersStats.get(left.getPlayerId()); stats2.setTimeLeft(left.getTime());
	 * stats2.addMinutes(); stats2.setTimeEntered(12); }); }
	 */
	...
	private void calculatePlayersPercentage(Match match) {
		...
		for (Map.Entry<String, PlayerStats> entry : playersStats.entrySet()) {
			...
			stats.summary();
			PlayerStats team = stats.getTeam().getTeamId().equals(match.getHomeTeam().getTeamId()) ? homeTeam
					: awayTeam;
	...
	private void returnPlayersStats(List<MatchEvent> matchEvents, String quarter) {
		matchEvents.stream().filter(x -> x.getEventType() == MatchEventType.SHOOTFOR2).forEach(x -> {
			...
			if (x.getSecondPlayer() != null)
				playersStats.get(x.getSecondPlayer().getPlayerId()).addAssistOrBlock(x.getPointsMade() == 2);
		});
		;
		matchEvents.stream().filter(x -> x.getEventType() == MatchEventType.SHOOTFOR3).forEach(x -> {
			...
			if (x.getSecondPlayer() != null)
				playersStats.get(x.getSecondPlayer().getPlayerId()).addAssistOrBlock(x.getPointsMade() != 3);
		});
		;
	...
	public Map<String, PlayerStats> returnPlayersStatsIndividual(Match match, List<String> homePlayersId,
			List<String> awayPlayersId) {
