...
import com.gameservice.util.ValueComparator;
...
public class MatchStatsCalculator {
	...
	// u teamStatsSummary se smesta summary stats tima jer je model isti samo za
	// playerId stavljam
	// teamName
	private Match match;
	...
	public void calculateMatchStats(Match match, String sortType) {
		...
		playersStats = scalc.returnPlayersStatsIndividual(match, match.getHomePlayersID(), match.getAwayPlayersID());
		summaryStats();
	...
	private void createLineScore() {
		...
		for (Quarter q : match.getQuarters()) {
			lineScore.put(match.getAwayTeam().getName(), q.getName(), q.getPointsAwayTeam() - awayTeamPoints);
			lineScore.put(match.getHomeTeam().getName(), q.getName(), q.getPointsHomeTeam() - homeTeamPoints);
	...
	public void sortByPoints() {
		...
		for (int i = 0; i < 5; i++) {
			map.put(awayTeamPlayerStats.get(i).getPlayer().getPlayerId(), awayTeamPlayerStats.get(i).getPoints());
			map.put(homeTeamPlayerStats.get(i).getPlayer().getPlayerId(), homeTeamPlayerStats.get(i).getPoints());
		}
		sortBestPlayers(map);
	}
	...
	public void sortByAssists() {
		...
		HashMap<String, Integer> map = new HashMap<String, Integer>();
		for (int i = 0; i < 5; i++) {
			map.put(awayTeamPlayerStats.get(i).getPlayer().getPlayerId(), awayTeamPlayerStats.get(i).getAssists());
			map.put(homeTeamPlayerStats.get(i).getPlayer().getPlayerId(), homeTeamPlayerStats.get(i).getAssists());
		}
		sortBestPlayers(map);
	}
