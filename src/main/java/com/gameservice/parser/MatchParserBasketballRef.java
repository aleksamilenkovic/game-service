...
import com.gameservice.parser.threads.MonthMatchParserThreads;
import com.gameservice.parser.threads.SeasonMatchParserThreads;
import com.gameservice.util.ConvertHelper;
import com.gameservice.util.JsoupHelper;
import com.gameservice.util.LoadPage;
import com.gameservice.util.MatchEventHelper;
...
public class MatchParserBasketballRef {
	...
	public MatchEvent returnMatchEvent(Element row, Quarter quarter) {
		...
		String time = cols.get(0).text(), awayTeamAction = cols.get(1).text(), scoreSummary, homeTeamAction, p;
		Player players[] = new Player[2];
		float timestamp = Float.parseFloat(time.substring(0, time.length() - 2).replace(':', '.'));
		int points = 0;
		boolean pointsMade = false;
		MatchEventType actionType = MatchEventType.NEUTRAL;
		...
		if (awayTeamAction.isEmpty() || awayTeamAction.equals("\u00a0")) {
			...
			matchEvent = MatchEvent.createForHomeTeam(scoreSummary, timestamp, points, homeTeamAction, actionType,
					players, quarter);
		} else {
			if (cols.size() == 2) {
				// neutralan dogadjaj
			...
			scoreSummary = cols.get(3).text();
			matchEvent = MatchEvent.createForAwayTeam(scoreSummary, timestamp, points, awayTeamAction, actionType,
					players, quarter);
		}
		...
		for (Element row : rows) {
			...
			if (row.select("th").size() > 0) {
				if (row.childNodeSize() == 3) {
					...
					// desava se da ide npr. 3.cetvrtina pa 4., pa opet 3. pa 4. (ova petlja ce
					// imati ne vise
	...
	public void returnPlayerIds(String matchId, List<String> homePlayers, List<String> awayPlayers, int year) {
		if (year < 2001)
			return;
		Document doc = null;
	...
	public Match returnMatch(String matchId, String fileName) {
		Match match = null;
		Document doc = null;
		LocalDateTime date = ConvertHelper.convertStringToLocalDate(matchId.substring(0, 8));
		...
		try {
			doc = fileName == null ? JsoupHelper.connectToLivePage(url) : JsoupHelper.connectToLocalPage(fileName);
		} catch (UrlException e) {
			System.out.println(e);
			return match;
		}
		...
		String pointsAwayTeam = teamPoints.select("div").get(0).text(), pointsHomeTeam = teamPoints.get(1).text();
		String dt = doc.select("#content > div.scorebox > div.scorebox_meta > div:nth-child(1)").first().text();
		LocalDateTime datetime = year < 2001 ? date : ConvertHelper.convertStringWithFullMonthToLocalDate(dt);
		Team awayTeam = Team.builder().name(teamNamesDiv.get(0).text())
		...
		match = new Match(datetime, awayTeam, pointsAwayTeam, homeTeam, pointsHomeTeam, new ArrayList<Quarter>(),
				matchId, year);
	...
	public List<Match> returnMatchesFromMonth(int year, String month) {
		...
		Document doc = null;
		try {
			doc = JsoupHelper.connectToLivePage(String.format(
					"https://www.basketball-reference.com/leagues/NBA_%d_games-%s.html", year, month.toLowerCase()));
		} catch (UrlException e) {
			System.out.println(e);
			return matches;
		}
		...
		/*
		 * Elements rows = doc.select("table#schedule tbody").first().select("tr"); for
		 * (int i = 0; i < rows.size(); i++) { Element row = rows.get(i); Elements cols
		 * = row.select("td"); if (cols.isEmpty() || cols.get(cols.size() -
		 * 4).text().isEmpty()) continue;
		 * 
		 * String matchId = cols.get(cols.size() - 4).select("a").attr("href"); matchId
		 * = matchId.substring(11, matchId.length() - 5); //
		 * System.out.println(matchId); Match match = returnMatch(matchId, null, year);
		 * matches.add(match); }
		 */
	...
	public Season returnSeasonWithMatches(int year) throws UrlException {
		...
		String url = String.format("https://www.basketball-reference.com/leagues/NBA_%d_games.html", year);
		 ...
		 */
		// }
		Season season = Season.builder().seasonYear(year).seasonMatches(seasonMatches).teams(null).build();
